...
import { AssetsItem, HSResponse, TraysItem } from '../@types/hotstar/pageTray'
...
export const getPageTray = async (
  ...
  const resultTray = items.flatMap(
    (trayItem) =>
      trayItem.assets?.items?.map((a) => AssetMap(a, trayItem)) || []
  )
...
const AssetMap = (assets: AssetsItem, trayItem?: TraysItem) => {
  ...
  const obj: Prisma.AssetCreateInput = {
    ...
    productionHouse: assets.productionHouse || null,
    ...
    broadCastDate: assets.broadCastDate
      ? new Date(assets.broadCastDate * 1000)
      : null,
    genre: assets.genre,
    AssetLang: {
      connectOrCreate: assets.langObjs
        ? Object.values(assets.langObjs).map((v) => ({
            where: {
              assetId_langId: { langId: v.id, assetId: +assets.contentId },
            },
            create: {
              lang: {
                connectOrCreate: {
  ...
  Object.keys(obj).forEach((key) => {
    ...
    if (Array.isArray(obj[key]))
      // @ts-expect-error
      obj[key] = obj[key].filter((x) => x !== undefined)
    // @ts-expect-error
